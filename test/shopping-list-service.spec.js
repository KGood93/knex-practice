...
describe(`Shopping List Service object`, function() {
    ...
    context(`Given 'shopping_list' has data`, () => {
        ...
        it(`getById() resolves an article by id from 'shopping_list table`, () => {
            const idToGet = 3
            const thridItem = testItems[idToGet -1]
            return ShoppingListService.getById(db, idToGet)
                .then(actual => {
                    expect(actual).to.eql({
                        id: idToGet, 
                        name: thridItem.name,
                        date_added: thridItem.date_added,
                        price: thridItem.price,
                        category: thridItem.category,
                        checked: false,
                    })
                })
        })

        it(`deleteItem() removes an aritcle by id from 'shopping_list' table`, () => {
            const articleId = 3
    ...
    context(`Given 'shopping_list' has no data`, () => {
        ...
        it(`insertItem() inserts an article and resolves the article with an 'id'`, () => {
            ...
            return ShoppingListService.instertItem(db, newItem)
                .then(actual => {
                    expect(actual).to.eql({
                        id: 1,
                        name: newItem.name,
